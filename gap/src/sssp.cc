...
#define INTERESTING_BOUNDARY 50000
...
void RelaxEdges(const WGraph &g, NodeID u, WeightT delta,
  ...
  #endif 
  #ifdef HBMPF
  WNode* end_wn = g.out_neigh(u).end() - g.out_neigh(u).begin() > 128 ? g.out_neigh(u).begin()+128 : g.out_neigh(u).end(); 
  for (WNode* wn = g.out_neigh(u).begin(); wn < end_wn; wn++) {
    // __builtin_prefetch(wn); 
    __builtin_prefetch(&dist[(wn)->v]); 
  }
  #endif 
