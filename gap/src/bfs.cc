...
// static uint64_t loop_time1; 
// static uint64_t loop_time2; 
...
int64_t BUStep(const Graph &g, pvector<NodeID> &parent, Bitmap &front,
  ...
  for (NodeID u=0; u < g.num_nodes(); u++) { // 134217728 iter per invoc 
    if (parent[u] < 0) {
      #ifdef INNER_COUNT
      uint64_t count = 0; 
      #endif 
      ...
      // for (NodeID v : g.in_neigh(u)) {
        ...
        if (v + 64 < g.in_neigh(u).end()) {
          ...
          front.prefetch_bit(*(v + 32)); 
        }
        #endif  
        if (front.get_bit(*v)) { // get_bit 26.5, 66.8% 
          parent[u] = *v;
          awake_count++;
          next.set_bit(u);
          break;
  ...
  while (!queue.empty()) {
    if (scout_count > edges_to_check / alpha) {
      ...
      do {
        ...
        // loop_time1 += chrono::duration_cast<chrono::microseconds>(end - start).count(); 
        front.swap(curr);
        t.Stop();
        if (logging_enabled)
      ...
      // const auto end = chrono::high_resolution_clock::now();
      // loop_time2 += chrono::duration_cast<chrono::microseconds>(end - start).count(); 
      queue.slide_window();
      t.Stop();
      if (logging_enabled)
